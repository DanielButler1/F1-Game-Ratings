...
export default function TeamsPageClient() {
	...
	const latestGameVersions =
		games.find((g) => g.gameName === latestGame)?.versions || [];
	const latestVersionForLatestGame =
		latestGameVersions[latestGameVersions.length - 1] || "B";
	...
	const selectedGame =
		query.game && games.some((g) => g.gameName === query.game)
			? query.game
			: latestGame;
	...
	const latestVersionForSelectedGame =
		versions[versions.length - 1] || latestVersionForLatestGame;

	const selectedVersion =
		query.version && versions.includes(query.version)
			? query.version
			: latestVersionForSelectedGame;
	...
	const handleGameChange = (game: string) => {
		...
		setQuery({
			game,
			version: nextGameVersions[nextGameVersions.length - 1] || "B",
		});
	};
	...
	const teamStats: TeamStats[] =
		selectedGame && selectedVersion
			? Object.entries(
					...
					.sort((a, b) => b.averageStats.overall - a.averageStats.overall)
			: [];
				...
				<Card className="p-6">
					<div className="grid gap-4 md:grid-cols-2">
						...
						<div className="space-y-2">
							...
							<Select
								onValueChange={(version) => setQuery({ version })}
								...
								<SelectContent>
									{versions.map((version) => (
										<SelectItem key={version} value={version}>
											{getVersionLabel(selectedGame, version)}
										</SelectItem>
									))}
								</SelectContent>
							</Select>
													...
													{drivers[0] && drivers[1] && (
														<>
															<div className="flex items-center gap-2">
																...
																<ComparisonBar
																	leftValue={drivers[0].overall}
																	rightValue={drivers[1].overall}
																	className="h-2"
																/>
															</div>
															<div className="flex items-center gap-2">
																...
																<ComparisonBar
																	leftValue={drivers[0].experience}
																	rightValue={drivers[1].experience}
																	className="h-2"
																/>
															</div>
															<div className="flex items-center gap-2">
																...
																<ComparisonBar
																	leftValue={drivers[0].racecraft}
																	rightValue={drivers[1].racecraft}
																	className="h-2"
																/>
															</div>
															<div className="flex items-center gap-2">
																...
																<ComparisonBar
																	leftValue={drivers[0].awareness}
																	rightValue={drivers[1].awareness}
																	className="h-2"
																/>
															</div>
															<div className="flex items-center gap-2">
																...
																<ComparisonBar
																	leftValue={drivers[0].pace}
																	rightValue={drivers[1].pace}
																	className="h-2"
																/>
															</div>
														</>
